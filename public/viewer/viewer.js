...
DV.backbone.DocumentViewer = Backbone.View.extend({
  initialize: function(options) {
    options = options || {};
  ...
  load: function(resource, options) {
    ...
    var doc = DV.backbone.model.Document();
    var documents = new DV.backbone.model.DocumentSet( [doc], options );
    var viewer = this.loadModels( documents, options );
    doc.on('reset', viewer.render, viewer);
    doc.fetch({ url: resource });
    return viewer;
  },
  
  loadDocuments: function(resource, options) {
    options = _.extend({}, options);
    var documents = new DV.backbone.model.DocumentSet([], _.extend(options, {url: resource}) );
    var viewer = this.loadModels(documents, options);
    documents.on('reset', viewer.render, viewer);
    documents.fetch({ url: resource });
    return viewer;
  },
  
  loadModels: function(documents, options) { 
    var viewer = new this(_.extend({}, options, {documents: documents}));
    return viewer;
  }
});
