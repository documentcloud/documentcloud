...
class DocumentsControllerTest < ActionController::TestCase
  ...
  def test_it_can_log_in_a_reviewer
    ...
    get :show, params: { :id=>doc.id, :format=>:html, :key=>reviewer.security_key.key }
    assert_response :success
    assert_equal reviewer.id, session['account_id']
  end

  def test_it_renders_a_public_document
    get :show, params: { :id=>doc.id, :format=>:html }
    assert_response :success
  ...
  def test_it_renders_a_private_document_to_owner
    get :show, params: { :id=>secret_doc.id, :format=>:html }
  ...
  def test_update
    login_account!
    put :update, params: { :id=>doc.id, :title=>'A new Title' }
    assert_response :success
  ...
  def test_redact_pages
    login_account!
    post :redact_pages, params: { :id=>doc.id, :redactions=>'[{"location":"286,614,353,437","page":1}]', :color=>'black' }
    assert_response :success
  ...
  def test_remove_pages
    login_account!
    post :remove_pages, params: { :id=>doc.id, :pages=>[1,2] }
    assert_response :success
  ...
  def test_reorder_pages
    login_account!
    post :reorder_pages, params: { :id=>doc.id, :page_order=> (0...doc.page_count).to_a.shuffle }
    assert_response :success
  ...
  def test_upload_insert_document
    login_account!
    post :upload_insert_document, params: { :id=>doc.id }
    assert_response 409
    post :upload_insert_document, params: { :id=>doc.id, :file=>'none', :replace_pages_start=>1, :document_number=>1, :document_count=>1 }
    assert_response :success
    assert_job_action 'document_remove_pages'
  end
  ...
  def test_save_page_text
    login_account!
    post :save_page_text, params: { :id=>doc.id, :modified_pages=>'{"1":"new page text"}' }
    assert_response :success
  ...
  def test_entities
    login_account!
    get :entities, params: { :ids=>[doc.id] }
  ...
  def test_entity
    login_account!
    get :entity, params: { :entity_id=>entities(:person).id }
    assert_response :success
  ...
  def test_retrieving_occurrence
    login_account!
    get :occurrence, params: { :id=>entities(:person).id, :occurrence=>'8:16' }
    assert_response :success
  ...
  def test_mentions
    login_account!
    get :mentions, params: { :id=>doc.id, :q=>'Ishmael' }
    assert_response :success
  ...
  def test_per_page_note_counts
    login_account!
    get :per_page_note_counts, params: { :id=>doc.id }
    assert_response :success
  ...
  def test_send_page_text
    ...
    get :send_page_text, params: { :id=>doc.id, :page_name=>'one-p88-p1-800' }
    assert_response :success
  ...
  def test_set_page_text
    login_account!
    post :set_page_text, params: { :id=>doc.id, :page_name=>'one-p1-800', :text=>'This is my page!' }
    assert_equal "This is my page!", pages(:first).text
  end

  def test_preview
    login_account!
    get :preview, params: { :id=>doc.id }
    assert_response :success
