...
class ProjectsControllerTest < ActionController::TestCase
  ...
  def test_create
    assert_difference 'Project.count' do
      post :create, params: { :title=>"New Project", :description=>"meh", :document_ids=>[secret_doc.id] }
    end
  end

  def test_update
    project = projects(:collab)
    put :update, params: { :title=>'rutabagas', :id=>project.id }
    assert_equal 'rutabagas', project.reload.title
  end

  def test_destroy
    assert_difference( 'Project.count', -1 ) do
      delete :destroy, params: { :id=>projects(:collab) }
    end
  end

  def test_documents
    get :documents, params: { :id=>projects(:collab) }
  ...
  def test_add_documents
    ...
    assert_difference 'project.documents.count' do
      put :add_documents, params: { :id=>project, :document_ids=>[secret_doc.id] }
    end
  end
  ...
  def test_remove_documents
    ...
    assert_difference( 'project.documents.count', -1 ) do
      put :remove_documents, params: { :id=>project, :document_ids=>[doc.id] }
    end
